...
for timeslice_ind in range(num_slices):
	...
	for i in range(Nv):
		...
		# Read in the time series for user i.
		...
		for j in range(i + 1, Nv):
			...
			# Keep track of the counts of 0s and 1s for
			# computing the joint distribution.

			count_array = collections.defaultdict(int)

			# Read in the time series for user j.
	...
	wfile = open('adj_mat_weighted{}.txt'.format(timeslice_ind), 'w')
