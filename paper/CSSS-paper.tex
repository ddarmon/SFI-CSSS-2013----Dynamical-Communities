...
%As a direct consquence these social media websites (e.g., Facebook and Twitter) constitute
 perfect laboratory to study these questions as they arise. For the purposes of this paper we choose to use Twitter as all of these datasets are permanently available to the public.
...
the mathematical theory of \emph{graphs}. In mathematics a \emph{graph} $G(V,E)$ is defined to be a set of so called \emph{vertices} $V$, (a.k.a nodes) as well as the connections between those vertices, known as \emph{edges} $E$ (a.k.a. links). Extending this idea to a network is trivial; In particular you can view a \emph{network} simply as a graph in which the nodes are actors in your system and the edges are the relationship between them. For example in a ``social-network" the nodes are the individuals and the edges are the relationships between those individuals. While this mathematical representation of a social network seems quite rudimentary, this representation yields rich, complex, and interesting properties of these networks. 

One such property is the existence of \emph{communities}: subsets of nodes who share many more edges between themselves than they share with the rest of the network. Detecting and classifying these communities is a very active area of research in network theory.  While many methods have been devised for detecting these communities from the networks \emph{structure} itself (for a nice review see \cite{fortunato2010community}),  effectively detecting \emph{dynamical/functional} communities is not as well researched and this is the primary focus of this paper. There are multitudes of other interesting properties from graph theory we can use to analyze a social-network (e.g., \emph{centrality}, \emph{betweenness}, \emph{average shortest path}, etc.) however what we find more interesting are the two more theoretical questions:  \emph{How does one properly define a network?} and \emph{What constitutes a community within that network?}  [[David: below in the comments is a discussion of random properties that luis had, feel free to add those back in if you feel like it motivates this paper better]] 
...
%we introduce. 
We further analyze the resulting communities and analyze structural changes in these networks over time in \ref{sec:discussion}. Finally we conclude and provide areas of future work in Section \ref{sec:futurework}.
...
is derived from a toy model and the other one is reconstructed from Twitter, a microblogging service
whose network is provided by users \emph{following} each other. We propose that a weighting of the
links with the information transfer between any two connected nodes might provide a refinement of
the network, and that this refinement brings us closer to the underlying constituents of the
networks. Let us note that the toy model was generated ad hoc with a desired and known number of
communities, whose belonging to them or not would determine the activity of each node as a
consequence of the activity of the others. In the real-world network the number of communities--if
any--is unknown.

    We searched for community structure in the original networks and in the weighted ones using an
algorithm whose performance has been assessed by other authors \cite{} and that is adequate to deal
with a great number of nodes. In the former, for the toy model the community structure is fragmented
and different from the one known by construction. Meanwhile, the algorithm applied upon the weighted
network faithfully recovers the structure that we had imposed. In the case of Twitter users, the
communities detected in the original network appear less fragmented than the ones we recover after
...
the difficulty in them before claiming any definitive resolution. We wonder ourselves what these
very complex networks are: both in the toy model and the twitter case the constituents were brought
...
these results to the real world network we must admit that weighting the links based on the activity
of the nodes reveals the object under research in a deeper, more adequate manner.
